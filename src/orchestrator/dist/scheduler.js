...
async function findScheduledFlows() {
  ...
  `);
    return result.rows.map(row => ({
        ...
        cronExpression: JSON.parse(row.node_data).config.cron_expression,
    }));
}
export async function registerScheduledFlows() {
    const flows = await findScheduledFlows();
    for (const flow of flows) {
        await executeFlowQueue.upsertJobScheduler(`schedule:${flow.flowId}`, // stable ID — upsert is idempotent
        { pattern: flow.cronExpression }, { name: 'execute_flow', data: { flowId: flow.flowId, tenantId: flow.tenantId } });
