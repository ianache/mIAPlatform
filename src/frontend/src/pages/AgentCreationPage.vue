<template>
  <div class="max-w-6xl mx-auto space-y-8">
    <!-- Page Header -->
    <div>
      <h1 class="text-3xl font-headline font-semibold text-primary">Create New Agent</h1>
      <p class="mt-1 text-sm text-onSurface-variant font-body">
        Configure your custom AI agent with identity, model, role, and capabilities.
      </p>
    </div>
    ...
    <form @submit.prevent="handleSubmit" novalidate>
      ...
      <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-8">
        ...
        >
          ...
          {{ agentStore.loading ? 'Creating...' : 'Create Agent' }}
        </button>
      </div>
    </form>
  </div>
...
function validateForm(): boolean {
  ...
  const nameOk =
    form.name.trim().length >= 3 && form.name.trim().length <= 100;
