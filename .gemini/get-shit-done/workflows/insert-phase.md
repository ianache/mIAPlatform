...
```bash
INIT=$(node "D:/02-PERSONAL/01-PROJECTS/22-mIAPlatform/.gemini/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "D:/02-PERSONAL/01-PROJECTS/22-mIAPlatform/.gemini/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
